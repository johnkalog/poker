...
export class PlayingCards {
  ...
  constructor(cards = null, from = 0, to = 0, firstid = 0) { //h parinas tis cards alliws ftiaxnei auto
    ...
    this.id=firstid;
    this.cards = _.shuffle(cardsRange).map( el=> { return {...el,toggled: false,id: this.id++ } }); //pio poly shmasia to epanw?
  ...
  export const getNCardsAndRest = (pcards, n, firstid) => { //pairnei kartes epistrefei tis upoloipes apo trapoula
    return {
  ...
  const getOfSameRank = (pcards, n) => { return pcards.rankTimes[n] || []; }  //exei n idia rank

  const getOfSameSuit = (pcards, n) => { return pcards.suitTimes[n] || []; }

  const hasAce = (pcards) => { return !!pcards.ranks['A']; }

  const hasOfSameRank = (pcards, n) => { return getOfSameRank(pcards,n).length; } //returns the length of the n same cards cpx exw 3-2ades h kai alla

  const hasOfSameSuit = (pcards, n) => { return getOfSameSuit(pcards,n).length; }

  const hasInARow = (pcards, n) => { return pcards.maxInARow >= n; }

  const getWorstSingles = (pcards) => { return _.sortBy(_.flatten(getOfSameRank(pcards,1)),'weight'); } //in ascending order the sigle cards by their weight.
